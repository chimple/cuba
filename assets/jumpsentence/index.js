  ...
  keyboardbutton: [ function(require, module, exports) {
    ...
    var KeyboardButton = function(_super) {
      ...
      KeyboardButton.prototype.onCollisionStay = function(other, self) {
        ...
        var currentIndex = this.bridgeComp.answerIndices.size - this.bridgeComp.count;
        if (this.collidingBox.getComponent(emptyBox_1.default).myCharacter === this.node.getChildByName("Label").getComponent(cc.Label).string && Array.from(this.bridgeComp.answerIndices.values())[currentIndex] === this.collidingBox.getComponent(emptyBox_1.default).characterIndex) {
      ...
      KeyboardButton.prototype.onTouchEnd = function(touch) {
        if (0 == touch.getID()) if (this.isProcessing) {
          ...
          if (this.collidingBox.getComponent(emptyBox_1.default).myCharacter === this.node.getChildByName("Label").getComponent(cc.Label).string && !this.isDone) {
            ...
            if ("en" == config_1.default.i.course.lang) {
              this.node.parent.parent.parent.emit("correct");
              var unlockedKeyboard = this.node.parent.parent.parent.getComponent(jumpsentence_1.default).unlockedKeyboard;
              if (unlockedKeyboard.active) {
                var keyboardAlphabets = unlockedKeyboard.getChildByName("Alphabets").getComponent(keyboardAlphabets_1.default);
                var currentIndex = this.bridgeComp.answerIndices.size - this.bridgeComp.count;
                var alphabetIndex = Array.from(this.bridgeComp.answerIndices.values())[currentIndex];
                var isUpperCase = function(str) {
                  return /^[A-Z]*$/.test(str) ? language_1.Case.Upper : language_1.Case.Lower;
                };
                isUpperCase(this.bridgeComp.positions[alphabetIndex]) !== keyboardAlphabets.case && keyboardAlphabets.onClickSwitchCaseButton();
              }
            } else this.node.parent.emit("correct");
