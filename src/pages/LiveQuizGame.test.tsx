...
jest.mock("../components/liveQuiz/LiveQuizQuestion", () => (props: any) => {
  ...
  return (
    <div data-testid="live-quiz-question">
      ...
      <button onClick={() => props.onNewQuestionChange?.(2)}>question-change</button>
      <button onClick={() => props.onRemainingTimeChange?.(11)}>remaining-change</button>
      <button onClick={() => props.onShowAnswer?.(true)}>show-answer</button>
      <button onClick={() => props.onConfigLoaded?.({ data: [{ question: { id: "q-1" } }] })}>
        config-loaded
      </button>
      <button onClick={() => props.onTotalScoreChange?.(74)}>score-change</button>
    </div>
  );
});
...
jest.mock("../components/liveQuiz/LiveQuizCountdownTimer", () => (props: any) => (
...
jest.mock("../components/parent/ScoreCard", () => (props: any) => {
  ...
  return (
    <div data-testid="scorecard">
      <button onClick={() => props.onContinueButtonClicked?.()}>continue</button>
...
describe("LiveQuizGame page", () => {
  beforeEach(() => {
    ...
    mockApi.getLesson.mockResolvedValue({ id: "lesson-from-room", name: "Room Lesson" });
  ...
  test("registers live quiz listener when liveRoomId exists", async () => {
    ...
    render(<LiveQuizGame />);
    await waitFor(() => {
      expect(mockApi.liveQuizListener).toHaveBeenCalledWith("room-1", expect.any(Function));
    });
  });
  ...
  test("renders countdown before timeout in room mode", async () => {
    ...
    expect(await screen.findByTestId("live-quiz-countdown")).toBeInTheDocument();
  });
  ...
  test("hides countdown after timeout and keeps question rendered", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId("live-quiz-countdown")).not.toBeInTheDocument();
  ...
  test("renders lesson mode question when lessonId query exists", async () => {
    ...
    mockState = {
      lesson: JSON.stringify({ id: "lesson-1", chapter_id: "ch-1", name: "Lesson 1" }),
      courseId: "course-1",
    };
    render(<LiveQuizGame />);
  ...
  test("passes lesson mode flags to LiveQuizQuestion", async () => {
    ...
    render(<LiveQuizGame />);
    await waitFor(() => {
      const props = mockQuestionSpy.mock.calls[mockQuestionSpy.mock.calls.length - 1][0];
  ...
  test("passes latest score to scorecard", async () => {
    ...
    await waitFor(() => {
      const props = mockScoreCardSpy.mock.calls[mockScoreCardSpy.mock.calls.length - 1][0];
  ...
  test("continue adds isReload=true for web when param exists and from has query", async () => {
    ...
    fireEvent.click(await screen.findByText("continue"));
    await waitFor(() => {
      expect(mockReplace).toHaveBeenCalledWith("/home?tab=ASSIGNMENT&isReload=true", mockState);
    });
  });
  ...
  test("continue adds isReload=true for web when from has no query", async () => {
    ...
    fireEvent.click(await screen.findByText("continue"));
    await waitFor(() => {
      expect(mockReplace).toHaveBeenCalledWith("/home?isReload=true", mockState);
    });
  });
