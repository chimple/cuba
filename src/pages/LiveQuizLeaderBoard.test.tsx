...
jest.mock("@ionic/react", () => ({
  ...
  IonRow: (props: any) => <div className={props.className}>{props.children}</div>,
}));
...
jest.mock("../components/common/StudentAvatar", () => (props: any) => (
  <div data-testid={`student-avatar-${props.student.id}`}>{props.student.name}</div>
));
...
describe("LiveQuizLeaderBoard page", () => {
  ...
  test("renders leaderboard header labels", async () => {
    render(<LiveQuizLeaderBoard />);
    expect(await screen.findByText("Live Quiz Leaderboard")).toBeInTheDocument();
  ...
  test("handles room doc without results gracefully", async () => {
    mockApi.getLiveQuizRoomDoc.mockResolvedValue({ assignment_id: "a-1", results: null });
    render(<LiveQuizLeaderBoard />);
    await waitFor(() => {
  ...
  test("does not render students missing from user_data map", async () => {
    mockApi.getStudentResultsByAssignmentId.mockResolvedValue([
      {
        user_data: [{ id: "s1", name: "One" }],
        result_data: [{ student_id: "s1", score: 10 }, { student_id: "s2", score: 9 }],
      },
    ]);
    render(<LiveQuizLeaderBoard />);
    await waitFor(() => {
  ...
  test("renders alternating row class names", async () => {
    const { container } = render(<LiveQuizLeaderBoard />);
    await waitFor(() => {
      expect(container.querySelectorAll(".leaderboard-row.even").length).toBeGreaterThan(0);
      expect(container.querySelectorAll(".leaderboard-row.odd").length).toBeGreaterThan(0);
    });
  });
    ...
    mockApi.getStudentResultsByAssignmentId.mockResolvedValue([
      {
        ...
        result_data: [{ student_id: "s1", score: 2 }, { student_id: "s2", score: 99 }],
      },
    ]);
    render(<LiveQuizLeaderBoard />);
    await waitFor(() => {
  ...
  test("supports single participant render", async () => {
    ...
    mockApi.getStudentResultsByAssignmentId.mockResolvedValue([
      { user_data: [{ id: "s1", name: "One" }], result_data: [{ student_id: "s1", score: 1 }] },
    ]);
    render(<LiveQuizLeaderBoard />);
    await waitFor(() => {
