...
jest.mock("../components/signup/LoginWithPhone", () => ({
  __esModule: true,
  default: ({ onNext, phoneNumber, setPhoneNumber, errorMessage, onFocus }: any) => (
    <div data-testid="phone-login">
      ...
      />
      {errorMessage ? <div data-testid="phone-error">{errorMessage}</div> : null}
      <button disabled={phoneNumber.length !== 10} onClick={onNext} type="button">phone-next</button>
      <button onClick={() => void onFocus()} type="button">phone-focus</button>
    </div>
  ),
}));
...
jest.mock("../components/signup/LoginWithStudentID", () => ({
  ...
  default: ({
    ...
    <div data-testid="student-login">
      <input aria-label="school-code-input" value={schoolCode} onChange={(e) => setSchoolCode(e.target.value)} />
      <input aria-label="student-id-input" value={studentId} onChange={(e) => setStudentId(e.target.value)} />
      <input aria-label="student-password-input" value={studentPassword} onChange={(e) => setStudentPassword(e.target.value)} />
      {errorMessage ? <div data-testid="student-error">{errorMessage}</div> : null}
      <button
        disabled={!schoolCode || !studentId || !studentPassword || studentPassword.length < 6}
...
jest.mock("../components/signup/LoginWithEmail", () => ({
  __esModule: true,
  default: ({ onLogin, onForgotPasswordChange, email, setEmail, password, setPassword, errorMessage }: any) => (
    <div data-testid="email-login">
      <input aria-label="email-input" value={email} onChange={(e) => setEmail(e.target.value)} />
      <input aria-label="password-input" value={password} onChange={(e) => setPassword(e.target.value)} />
      {errorMessage ? <div data-testid="email-error">{errorMessage}</div> : null}
      <button
        disabled={!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || password.length < 6 || /\s/.test(password)}
        onClick={() => void onLogin(email, password)}
        type="button"
      >
        email-login
      </button>
      <button onClick={onForgotPasswordChange} type="button">forgot-password</button>
    </div>
  ),
}));
...
jest.mock("../components/signup/OtpVerification", () => ({
  __esModule: true,
  default: ({ onVerify, verificationCode, setVerificationCode, errorMessage, phoneNumber }: any) => (
...
jest.mock("../components/signup/ForgotPass", () => ({
  __esModule: true,
  default: ({ onGoBack }: any) => <button onClick={onGoBack} type="button">forgot-go-back</button>,
}));

jest.mock("../components/signup/LoginSwitch", () => ({
  __esModule: true,
  default: ({ loginType, onSwitch, onCheckboxChange, checkbox, onResend, onGoogleSignIn, onTermsClick, showResendOtp, counter, otpExpiryCounter }: any) => (
    <div data-testid="login-switch">
      ...
      <div data-testid="switch-show-resend">{String(Boolean(showResendOtp))}</div>
      <div data-testid="switch-otp-expiry">{String(otpExpiryCounter)}</div>
      <button onClick={() => onSwitch("phone")} type="button">switch-phone</button>
      <button onClick={() => onSwitch("student")} type="button">switch-student</button>
      <button onClick={() => onSwitch("email")} type="button">switch-email</button>
      <button onClick={onTermsClick} type="button">open-terms</button>
      <button onClick={() => onCheckboxChange(!checkbox)} type="button">toggle-checkbox</button>
      <button onClick={() => checkbox && void onGoogleSignIn()} type="button">google-signin</button>
...
describe("LoginScreen", () => {
  beforeEach(() => {
    ...
    mockOnlineOfflineHandler.mockReturnValue({ online: true, presentToast: mockPresentToast });
  ...
  describe("Login Type + Basic UI", () => {
    ...
    it("updates language while staying on email login state", async () => {
      ...
      expect(view.getByLabelText("email-input")).toHaveValue("parent@example.com");
    ...
    it("updates language while staying on student login state", async () => {
      ...
      expect(view.getByLabelText("student-password-input")).toHaveValue("pass123");
    });
  ...
  describe("Phone + OTP", () => {
    ...
    it("does not submit for less than 10 digits and submits for valid 10-digit input", async () => {
      ...
      await user.click(startBtn);
      await eventually(() => {
        expect(mockAuthHandler.generateOtp).toHaveBeenCalledWith("1234567890", "Chimple");
      });
    });
    ...
    it("validates phone and handles OTP generation error states", async () => {
      ...
      (mockAuthHandler.generateOtp as jest.Mock).mockResolvedValueOnce({ success: false, error: "rate" });
      await user.clear(view.getByLabelText("phone-input"));
      await user.type(view.getByLabelText("phone-input"), "9876543210");
      await user.click(startBtn);
      expect(await view.findByTestId("phone-error")).toHaveTextContent("Kindly wait for 1 minute and then try logging in again.");

      (mockAuthHandler.generateOtp as jest.Mock).mockRejectedValueOnce(new Error("network"));
      await user.click(startBtn);
      await eventually(() => {
    ...
    it("shows entered phone number on OTP screen and back returns to phone input", async () => {
      ...
      await user.type(view.getByLabelText("phone-input"), "9876543210");
      await user.click(view.getByRole("button", { name: "phone-next" }));
      expect(await view.findByTestId("otp-phone")).toHaveTextContent("9876543210");
    ...
    it("does not verify for less than 6 digits and verifies for exactly 6 digits", async () => {
      ...
      expect(mockAuthHandler.proceedWithVerificationCode).not.toHaveBeenCalled();
    ...
    it("auto picks OTP from plugin event and triggers verification", async () => {
      ...
      await eventually(() => {
        expect(mockAuthHandler.proceedWithVerificationCode).toHaveBeenCalledWith("9876543210", "654321");
      });
      addListenerSpy.mockRestore();
    });
    ...
    it("handles OTP verify failure and resend behavior", async () => {
      (mockAuthHandler.proceedWithVerificationCode as jest.Mock).mockRejectedValue("code-expired");
      ...
      await user.click(view.getByRole("button", { name: "verify-otp" }));
      expect(await view.findByTestId("otp-error")).toHaveTextContent("Verification code has expired. Please request a new one.");

      await user.click(view.getByRole("button", { name: "resend-otp" }));
      await eventually(() => {
        expect(view.getByTestId("otp-error")).toHaveTextContent("Resend Otp Failed!! Please try again after some time.");
      });
    });
    ...
    it("shows wrong OTP message for invalid OTP", async () => {
      ...
      await user.type(view.getByLabelText("otp-input"), "111111");
      await user.click(view.getByRole("button", { name: "verify-otp" }));
      expect(await view.findByTestId("otp-error")).toHaveTextContent("Incorrect OTP - Please check & try again!");
    });
    ...
    it("resends OTP successfully when eligible", async () => {
      ...
      await user.click(view.getByRole("button", { name: "resend-otp" }));
      await eventually(() => {
        expect(mockAuthHandler.resendOtpMsg91).toHaveBeenCalledWith("9876543210");
      });
    });
  ...
  describe("Google Sign-In", () => {
    ...
    it("shows error and falls back to phone when google sign fails", async () => {
      ...
      await eventually(() => {
        expect(mockPresentToast).toHaveBeenCalled();
        expect(view.getByTestId("switch-login-type")).toHaveTextContent(LOGIN_TYPES.PHONE);
      });
    });
  });
  ...
  describe("Email Login", () => {
    ...
    it("handles email success and failure branches", async () => {
      ...
      await eventually(() => {
        expect(view.getByTestId("email-error")).toHaveTextContent("Incorrect credentials - Please check & try again!");
      });
    });
  });
  ...
  describe("Student Login", () => {
    ...
    it("submits student credentials format", async () => {
      ...
      await eventually(() => {
        expect(mockAuthHandler.loginWithEmailAndPassword).toHaveBeenCalledWith("SCH001@chimple.net", "pass123");
      });
    });
    ...
    it("shows student credential error on failed response", async () => {
      ...
      expect(await view.findByTestId("student-error")).toHaveTextContent("Incorrect credentials - Please check & try again!");
    });
