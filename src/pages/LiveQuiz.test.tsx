...
describe('LiveQuiz page', () => {
  beforeEach(() => {
    ...
    mockApi.getLesson.mockImplementation((id: string) =>
      Promise.resolve({ id, name: `Lesson ${id}` }),
    );
    mockApi.assignmentListner.mockImplementation(
      (_classId: string, cb: Function) => {
        (mockApi.assignmentListner as any).cb = cb;
      },
    );
    mockApi.assignmentUserListner.mockImplementation(
      (_studentId: string, cb: Function) => {
  ...
  test('passes sorted lessons to slider by assignment created date', async () => {
    ...
    await waitFor(() => {
      const call =
        mockLessonSliderSpy.mock.calls[
          mockLessonSliderSpy.mock.calls.length - 1
        ][0];
  ...
  test('shows empty state when linked class exists but no quizzes', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('You do not have any live quizzes available.'),
      ).toBeInTheDocument();
    });
  });
  ...
  test('debounces re-init for live-quiz assignment events', async () => {
    ...
    await waitFor(
      () => {
        expect(mockApi.getStudentResultInMap.mock.calls.length).toBeGreaterThan(
          initialGetCalls,
        );
      },
      { timeout: 2500 },
  ...
  test('assignment-user listener triggers re-init for live-quiz assignments', async () => {
    renderPage();
    await waitFor(() =>
      expect(mockApi.assignmentUserListner).toHaveBeenCalled(),
    );
    const initialGetCalls = mockApi.getStudentResultInMap.mock.calls.length;
