...
describe('LiveQuizRoomResult page', () => {
  ...
  test('renders top performer as first podium avatar', async () => {
    ...
    const firstCall = mockAvatarSpy.mock.calls.find(
      (c) => c[0].nameLabel === '1st',
    );
  ...
  test('renders second and third podium labels', async () => {
    ...
    await waitFor(() => {
      expect(
        mockAvatarSpy.mock.calls.some((c) => c[0].nameLabel === '2nd'),
      ).toBe(true);
      expect(
        mockAvatarSpy.mock.calls.some((c) => c[0].nameLabel === '3rd'),
      ).toBe(true);
    });
  });
  ...
  test('breaks ties by lower total time spent', async () => {
    ...
    render(<LiveQuizRoomResult />);
    await waitFor(() => {
      const firstPodium = mockAvatarSpy.mock.calls.find(
        (c) => c[0].nameLabel === '1st',
      );
