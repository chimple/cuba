...
describe("Parent page", () => {
  beforeEach(() => {
    ...
    mockMergeStudentsByUpdatedAt.mockImplementation((profiles: any[]) => profiles);
  ...
  it("updates language via dropdown selection", async () => {
    ...
    await user.selectOptions(screen.getByLabelText("language-dropdown"), "lang-2");
  ...
  it("toggles Sound and Music and updates flags", async () => {
    ...
    await waitFor(() =>
      expect(mockSetCurrentSound).toHaveBeenCalledWith(0)
    );
    await waitFor(() =>
      expect(mockApiHandler.updateSoundFlag).toHaveBeenCalledWith("parent-1", true)
    );

    await user.click(screen.getByRole("button", { name: "Music" }));
    await waitFor(() =>
      expect(mockSetCurrentMusic).toHaveBeenCalledWith(0)
    );
    await waitFor(() =>
      expect(mockApiHandler.updateMusicFlag).toHaveBeenCalledWith("parent-1", true)
    );
  });
  ...
  it("renders Help tab and opens external links", async () => {
    ...
    expect(window.open).toHaveBeenCalledWith("https://www.chimple.org/", "_system");
  ...
  it("covers schools condition false paths (null, non-array, empty array)", async () => {
    ...
    await waitFor(() => expect(mockApiHandler.getSchoolsForUser).toHaveBeenCalledTimes(2));
    ...
    await waitFor(() => expect(mockApiHandler.getSchoolsForUser).toHaveBeenCalledTimes(3));
  });
  ...
  it("covers init early return when languages are empty and when not an array", async () => {
    ...
    await waitFor(() => expect(mockApiHandler.getAllLanguages).toHaveBeenCalledTimes(2));
  });
  ...
  it("language change: returns early when langDoc is not found, and skips updateLanguage when currentUser is null", async () => {
    ...
    await waitFor(() => expect(mockApiHandler.updateLanguage).not.toHaveBeenCalled());
