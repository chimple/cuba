...
function getColor(
  ...
  if (
    ...
    if (
      ...
      studentResults.some((r) => r.score !== null)
    ) {
      let totalScore = studentResults.reduce(
      ...
      {Object.keys(studentData).map((assignmentId) => {
        ...
        let isIndividuallyAssigned = (assignmentUserRecords ?? []).some(
          (record) =>
            ...
            studentData[assignmentId]?.some(
              (r) => r.student_id === record.user_id,
            ),
        );

        if (
          !isIndividuallyAssigned &&
          studentData[assignmentId]?.length > 0 &&
          !belongsToClass
        ) {
        ...
        if (currentResults.length > 0) {
          if (isScore) {
            displayValue =
              currentResults[0].score !== null
                ? currentResults[0].score.toString()
                : '';
          } else {
