...
describe("CreateSelectedAssignment (QR flow)", () => {
  beforeEach(() => {
    ...
    const groupedStudents = new Map([
      [BANDS.REDGROUP, [{ id: "student-1", name: "Student 1", selected: true }]],
      [BANDS.YELLOWGROUP, []],
      [BANDS.GREENGROUP, []],
      [BANDS.GREYGROUP, []],
    ]);
    ...
    (ClassUtil as unknown as jest.Mock).mockImplementation(() => ({
      ...
      groupStudentsByCategoryInList: jest.fn().mockResolvedValue(groupedStudents),
    }));

    mockAuth.getCurrentUser.mockResolvedValue({ id: "teacher-1" });
    mockApi.getCoursesForClassStudent.mockResolvedValue([{ id: "course-1" }]);
    mockApi.getUserAssignmentCart.mockResolvedValue({ lessons: JSON.stringify({}) });
  ...
  test("falls back to getChapterByLesson and creates QR assignment with qr_code source", async () => {
    ...
    expect(mockApi.getChapterByLesson).toHaveBeenCalledWith("lesson-1", "class-1");
  ...
  test("toggles Select All to deselect all students and updates counts", async () => {
    const groupedStudents = new Map([
      ...
      [BANDS.YELLOWGROUP, [{ id: "student-3", name: "Student 3", selected: true }]],
      [BANDS.GREENGROUP, []],
      [BANDS.GREYGROUP, []],
    ]);
  ...
  test("deselecting a single student unchecks global Select All", async () => {
    ...
    const studentCheckbox = within(studentListItem as HTMLElement).getByRole("checkbox");
