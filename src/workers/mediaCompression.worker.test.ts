...
describe("mediaCompression.worker", () => {
  const setupCanvas = (convertToBlobImpl?: jest.Mock) => {
    (globalThis as unknown as { createImageBitmap: jest.Mock }).createImageBitmap =
      jest.fn(async () => ({
        width: 100,
        height: 50,
        close: jest.fn(),
    ...
    (globalThis as unknown as { OffscreenCanvas: any }).OffscreenCanvas = class {
      width = 0;
      height = 0;
      getContext() {
  ...
  const loadWorker = async () => {
    jest.resetModules();
    jest.clearAllMocks();
    (globalThis as unknown as { postMessage: jest.Mock }).postMessage = jest.fn();
    ...
    return (globalThis as unknown as { onmessage: (event: any) => Promise<void> })
      .onmessage;
  };
  ...
  test("compresses video and emits progress + done response", async () => {
    ...
    const calls = (globalThis as unknown as { postMessage: jest.Mock }).postMessage
      .mock.calls;
    expect(calls.length).toBeGreaterThan(1);
    const progressCall = calls.find(
      (call: unknown[]) => call[0] && typeof call[0] === "object" && "progress" in call[0],
    );
