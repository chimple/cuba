      ...
      if (supportsMultiThread) {
        try {
          ...
          try {
            ...
            await loadCore("https://unpkg.com/@ffmpeg/core-mt@0.12.6/dist/umd", true);
          }
        }
      } else {
        try {
          ...
          await loadCore("https://unpkg.com/@ffmpeg/core@0.12.6/dist/umd", false);
        }
      }
...
const compressVideo = async (
  id: string,
  payload: Extract<CompressWorkerRequest, { type: "COMPRESS_VIDEO" }>["payload"],
) => {
  ...
  return runWithFFmpegLock(async () => {
    ...
    try {
      ...
      const outBytes = output instanceof Uint8Array ? output : new TextEncoder().encode(output);
...
const compressImageCanvas = async (
  ...
  postProgress(id, 0.05, "loading");
  const blob = new Blob([payload.buffer], { type: payload.mimeType || "image/jpeg" });
