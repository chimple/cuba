...
export function renderPathwaySvgScene(params: any) {
  ...
  requestAnimationFrame(() => {
    ...
    renderHost.innerHTML = pathwaySVG;
    const svg = renderHost.querySelector('svg') as SVGSVGElement;
    if (!svg) {
      stopPathwayLoading();
      return;
    }
    ...
    for (let idx = 0; idx < lessons.length; idx += 1) {
      ...
      const isPlayed = startIndex + idx < currentIndex || activeIndex === -1;
      const isActive = startIndex + idx === currentIndex && activeIndex !== -1;
      ...
      if (isPlayed) {
        ...
        if (typeof halo === 'string') {
          ...
          const haloNode = halo.cloneNode(true) as SVGSVGElement | SVGGElement;
        ...
        fragment.appendChild(buildInactiveLessonNode(idx, flowerX, flowerY));
      }
      lastIndex = idx;
    }
    ...
    const playRewardClickAnimation = (
      ...
      const willOpenPreview =
        mode === 'sticker' && isStickerBookPreviewOn && stickerPreviewPayload;
      ...
      const animationClass = 'PathwayStructure-end-reward-box--sticker-clicked';
    ...
    const isStringReward =
      newRewardIdFromCheck !== null && typeof newRewardIdFromCheck === 'string';
