...
export class SupabaseApiProgramSchoolMetrics extends SupabaseApiProgramCatalog {
  // Converts the UI date range into the day count expected by metrics RPCs.
  private getSchoolMetricsDays(dateRange?: string | null) {
  ...
  private async getGradeIdsForSchoolListingFilter(params: {
    gradeValues: string[];
  }): Promise<string[]> {
  ...
  private async getSchoolListingMetricsByGrades(params: {
    ...
    const gradeValues = params.filters?.grade ?? [];
    const gradeIds = await this.getGradeIdsForSchoolListingFilter({
      gradeValues,
    });
    ...
    try {
      const { data, error } = await this.supabase.rpc(
        'get_school_listing_metrics_by_grades',
        {
          ...
          p_days: this.getSchoolMetricsDays(params.dateRange),
          p_page: params.page,
      ...
      if (error) {
        logger.error(
          'RPC error in get_school_listing_metrics_by_grades:',
          error,
        );
        return null;
      }
  ...
  async getSchoolMetricsForSchoolListing(params: {
    ...
    if (selectedGradeValues.length > 0) {
      const gradeMetricsResponse = await this.getSchoolListingMetricsByGrades({
        filters,
        programId,
        page,
        pageSize: page_size,
