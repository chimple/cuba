        ...
        for (const lesson of group.navigation) {
          if (lesson.cocosChapterCode === lessonId) {
            return {
              ...
              created_at: "",
              created_by: null,
              image: Util.getThumbnailUrl(lesson, "lesson"),
              is_deleted: null,
              language_id: null,
    ...
    try {
      ...
      const chapters: TableTypes<"chapter">[] = courseJson.groups.map(
        (group: any) => ({
          ...
          image: Util.getThumbnailUrl(group,"chapter"),
          course_id: courseId,
          created_at: null,
          updated_at: null,
          is_deleted: null,
        ...
        try {
          ...
          if (courseJson?.metadata) {
            ...
            const localCourse = {
              ...
              image: Util.getThumbnailUrl(metaC, "course"),
              is_deleted: false,
    ...
    try {
      ...
      const lessons: TableTypes<"lesson">[] = courseJson.groups.flatMap(group =>
        group.navigation.filter(lesson => lessonIds.includes(lesson.id)).map((lesson: any) => ({
          ...
          status: lesson.status,
          type: lesson.type,
          thumbnail: Util.getThumbnailUrl(lesson, "lesson"),
          plugin_type: lesson.pluginType,
          created_at: "null",
          updated_at: "null",
          is_deleted: null
