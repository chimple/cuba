        ...
        for (const lesson of group.navigation) {
          if (lesson.cocosChapterCode === lessonId) {
            return {
              ...
              created_at: "",
              created_by: null,
              image: Util.getThumbnailUrl(lesson.cocosSubjectCode,lesson.cocosLessonCode),
              is_deleted: null,
              language_id: null,
    ...
    try {
      ...
      const chapters: TableTypes<"chapter">[] = courseJson.groups.map(
        (group: any) => ({
          ...
          image: Util.getThumbnailUrl(undefined,undefined, group.metadata.id),
          course_id: courseId,
          created_at: null,
          updated_at: null,
          is_deleted: null,
        ...
        try {
          ...
          if (courseJson?.metadata) {
            ...
            const localCourse = {
              ...
              image: Util.getThumbnailUrl(undefined,undefined,undefined,metaC.courseCode),
              is_deleted: false,
          ...
          for (const lesson of group.navigation) {
            if (lesson.id === lessonId) {
              return {
                id: group.metadata.id,
                name: group.metadata.title,
                image: Util.getThumbnailUrl(undefined,undefined,undefined,group.metadata.id),
                course_id: courseId,
                created_at: "",
                updated_at: null,
                is_deleted: null,
    ...
    try {
      ...
      const lessons: TableTypes<"lesson">[] = courseJson.groups.flatMap(group =>
        group.navigation.filter(lesson => lessonIds.includes(lesson.id)).map((lesson: any) => ({
          ...
          status: lesson.status,
          type: lesson.type,
          thumbnail: Util.getThumbnailUrl(lesson.cocosSubjectCode, lesson.cocosLessonCode),
          plugin_type: lesson.pluginType,
          created_at: "null",
          updated_at: "null",
          is_deleted: null
