...
export class OneRosterApi implements ServiceApi {
  ...
  public async loadCourseJson(courseId: string) {
    try {
      ...
      logger.error(`Failed to load ${courseId}:`, error);
    }
  }
  ...
  async addCourseForParentsStudent(
    ...
    try {
      ...
      logger.error('Error adding courses:', error);
    }
  }
  ...
  async getLessonWithCocosLessonId(
    ...
    try {
      ...
      logger.error('Error in getLessonWithCocosLessonId:', error);
      return null;
    }
  }
  ...
  async getDifferentGradesForCourse(course: TableTypes<'course'>): Promise<{
    ...
    for (const c of allCourses) {
      try {
        ...
        logger.error(`Error loading course JSON for ${c.id}:`, error);
      }
    }
  ...
  async getCoursesForPathway(
    ...
    try {
      ...
      logger.error('Error parsing stored courses:', e);
      return [];
    }
  }
  ...
  > {
    try {
      ...
      logger.error('Error in getChaptersForCourse:', error);
  ...
  async getLessonsForChapter(
    ...
    try {
      ...
      logger.error('Error in getLessonsForChapter:', error);
      return []; // Return empty array on error
    }
  }
    ...
    try {
      ...
      for (let i = 0; i < this.studentAvailableCourseIds.length; i++) {
        ...
        try {
          ...
          logger.error(`Error processing course ${courseId}:`, e);
  ...
  async getStudentResult(
    ...
    try {
      ...
      logger.error('Error in getStudentResultInMap:', error);
  ...
  > {
    try {
      ...
      logger.error('Error in getStudentProgress:', error);
      return {};
    }
  }
  ...
  async getLessonResultsForStudent(
    ...
    try {
      ...
      logger.error('Error in getLessonResultsForStudent:', error);
      return undefined;
    }
  }
    ...
    try {
      ...
      for (const course of courses) {
        try {
          ...
          logger.error(`Error processing course ${course.id}:`, error);
          continue;
        }
      }
      ...
      logger.error('Error in searchLessons:', error);
      return [];
    }
  }
  ...
  public async getChapterIDByLessonID(
    ...
    try {
      ...
      logger.error('Error fetching chapter by lesson:', error);
      return undefined;
    }
  }
  ...
  async getFavouriteLessons(userId: string): Promise<TableTypes<'lesson'>[]> {
    try {
      ...
      logger.error('on getFavouriteLessons error', error);
      return [];
    }
  }
  ...
  async getRecommendedLessons(
    ...
    try {
      ...
      logger.error('Error in getRecommendedLessons:', error);
      return [];
    }
  }
