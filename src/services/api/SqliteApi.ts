...
async deleteStudentFromClass(
  ...
  try {
    ...
    /* ===============================
     * 2️⃣ Soft delete student mapping
     * =============================== */
    await this.executeQuery(
      `
      UPDATE class_user
      ...
      WHERE user_id = ?
        ...
        AND is_deleted = 0;
      `,
      [true, timestamp, studentId, classId]
    );

    // 🔥 PUSH CHANGE FOR STUDENT
    this.updatePushChanges(
      TABLES.ClassUser,
      MUTATE_TYPES.UPDATE,
      {
    ...
    const parentId = parentRes?.values?.[0]?.parent_id;

    if (parentId) {
      /* ===============================
       * 4️⃣ Check remaining siblings
       * =============================== */
      const siblingRes = await this.executeQuery(
        `
        SELECT COUNT(*) as remaining_count
        FROM parent_user pu
        JOIN class_user cu
          ON pu.student_id = cu.user_id
        WHERE pu.parent_id = ?
          AND pu.student_id != ?
          AND pu.is_deleted = 0
          AND cu.class_id = ?
          AND cu.is_deleted = 0;
        `,
        [parentId, studentId, classId]
      );

      const remainingCount =
        siblingRes?.values?.[0]?.remaining_count ?? 0;

      /* ===============================
       * 5️⃣ If no siblings → delete parent mapping
       * =============================== */
      if (Number(remainingCount) === 0) {
        await this.executeQuery(
          ...
          WHERE user_id = ?
            ...
            AND is_deleted = 0;
          `,
          [true, timestamp, parentId, classId]
        );

        // 🔥 PUSH CHANGE FOR PARENT
        this.updatePushChanges(
          TABLES.ClassUser,
          MUTATE_TYPES.UPDATE,
          {
    ...
    console.error("SQLite deleteStudentFromClass error:", error);
    throw error;
  }
}
}
