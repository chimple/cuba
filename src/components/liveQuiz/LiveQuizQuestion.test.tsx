...
describe("LiveQuizQuestion component", () => {
  ...
  const renderQuestion = (props?: Partial<React.ComponentProps<typeof LiveQuizQuestion>>) =>
    render(
      <LiveQuizQuestion
        lessonId="lesson-1"
  ...
  test("does not initialize when both roomDoc and lessonId are missing", async () => {
    render(
      <LiveQuizQuestion showQuiz={true} isTimeOut={true} onQuizEnd={jest.fn()} />
    );
    await waitFor(() => {
      expect(screen.queryByText("Which animal is known as the king of the jungle?")).not.toBeInTheDocument();
    });
  });

  test("renders first question text in web mode", async () => {
    renderQuestion();
    expect(await screen.findByText("Which animal is known as the king of the jungle?")).toBeInTheDocument();
  });
  ...
  test("reports initial remaining time for each question", async () => {
    ...
    await waitFor(() => {
      expect(onRemainingTimeChange).toHaveBeenCalledWith(LIVE_QUIZ_QUESTION_TIME);
    });
  });
  ...
  test("reports countdown ticks", async () => {
    ...
    await waitFor(() => {
      expect(onRemainingTimeChange).toHaveBeenCalledWith(LIVE_QUIZ_QUESTION_TIME - 1);
    });
  });
  ...
  test("plays TTS for question audio button when only text exists", async () => {
    ...
    const icons = container.querySelectorAll(".live-quiz-audio-button-question svg");
  ...
  test("plays TTS for option audio button when text option is clicked", async () => {
    const { container } = renderQuestion();
    await screen.findByText("Lion");
    const optionIcon = container.querySelector(".live-quiz-audio-button-option svg");
  ...
  test("hides question UI when showQuiz is false", async () => {
    ...
    await waitFor(() => {
      expect(container.querySelector(".live-quiz-question")).not.toBeInTheDocument();
      expect(container.querySelector(".live-quiz-options")).not.toBeInTheDocument();
    });
  });
  ...
  test("shows lesson mode top/bottom padding wrapper for dots", async () => {
    const { container } = renderQuestion();
    await screen.findByText("Lion");
    const wrapper = container.querySelector(".live-quiz-navigation-dots") as HTMLElement;
