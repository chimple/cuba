...
jest.mock("./LiveQuizStudentAvatar", () => (props: any) => (
  <div
    ...
    data-correct={props.isCorrect == null ? "undefined" : String(props.isCorrect)}
    data-percentage={props.percentage == null ? "undefined" : String(props.percentage)}
  >
    {props.student.name}
  </div>
));
...
describe("LiveQuizHeader", () => {
  beforeEach(() => {
    ...
    (Util.getCurrentStudent as jest.Mock).mockReturnValue({ id: "s2", name: "Me" });
  ...
  const renderHeader = (overrides?: any) =>
    render(
      <LiveQuizHeader
        ...
        currentQuestion={overrides?.currentQuestion ?? { id: "q2", text: "Q2?" }}
  ...
  test("moves current student to first position", async () => {
    ...
    await waitFor(() => {
      const ids = Array.from(container.querySelectorAll("[data-testid^='avatar-']")).map((n) =>
  ...
  test("uses live score when showAnswer is true", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("avatar-s1")).toHaveAttribute("data-score", "15");
      expect(screen.getByTestId("avatar-s2")).toHaveAttribute("data-score", "10");
    });
  });
    ...
    rerender(
      <LiveQuizHeader
        roomDoc={{
          ...
          results: {
            ...baseRoomDoc.results,
            s1: [...baseRoomDoc.results.s1, { question_id: "q3", score: 10, timeSpent: 3 }],
          },
        }}
        remainingTime={15}
        showAnswer={false}
  ...
  test("computes isCorrect true for correctly answered current question", async () => {
    renderHeader({ currentQuestion: { id: "q2", text: "Q2?" }, showAnswer: true });
    await waitFor(() => {
      expect(screen.getByTestId("avatar-s2")).toHaveAttribute("data-correct", "true");
    });
  });
  ...
  test("computes isCorrect false for wrong answer on current question", async () => {
    renderHeader({ currentQuestion: { id: "q2", text: "Q2?" }, showAnswer: true });
    await waitFor(() => {
      expect(screen.getByTestId("avatar-s1")).toHaveAttribute("data-correct", "true");
      expect(screen.getByTestId("avatar-s3")).toHaveAttribute("data-correct", "false");
    });
  });
  ...
  test("passes undefined isCorrect while answer is hidden", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("avatar-s1")).toHaveAttribute("data-correct", "undefined");
      expect(screen.getByTestId("avatar-s2")).toHaveAttribute("data-correct", "undefined");
    });
  });

  test("computes percentage based on remaining time when current question not answered yet", async () => {
    renderHeader({ currentQuestion: { id: "q99", text: "new" }, remainingTime: 10 });
    await waitFor(() => {
      const expected = ((LIVE_QUIZ_QUESTION_TIME - 10) / LIVE_QUIZ_QUESTION_TIME) * 100;
      expect(screen.getByTestId("avatar-s1")).toHaveAttribute("data-percentage", String(expected));
    });
  });

  test("omits percentage when current question matches last answered question", async () => {
    renderHeader({ currentQuestion: { id: "q2", text: "Q2?" }, remainingTime: 10 });
    await waitFor(() => {
      expect(screen.getByTestId("avatar-s1")).toHaveAttribute("data-percentage", "undefined");
    });
  });

  test("omits percentage when remaining time is undefined", async () => {
    renderHeader({ currentQuestion: { id: "q99", text: "Q99?" }, remainingTime: 0 });
    await waitFor(() => {
      expect(screen.getByTestId("avatar-s2")).toHaveAttribute("data-percentage", "undefined");
    });
  });
  ...
  test("renders rank circles when question index is greater than zero", async () => {
    ...
    await waitFor(() => {
      expect(container.querySelectorAll(".top-performer-circle").length).toBeGreaterThan(0);
    });
  });
  ...
  test("does not render rank circles when question index is zero", async () => {
    ...
    await waitFor(() => {
      expect(container.querySelectorAll(".top-performer-circle")).toHaveLength(0);
    });
  });
  ...
  test("supports participants with empty result arrays", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("avatar-s3")).toHaveAttribute("data-score", "undefined");
    });
  });
  ...
  test("handles empty participants list", async () => {
    const { container } = renderHeader({ roomDoc: { ...baseRoomDoc, participants: [] } });
    await waitFor(() => {
