...
describe("LiveQuizCountdownTimer", () => {
  ...
  const renderTimer = (secondsAhead: number, onTimeOut = jest.fn()) => {
    ...
    const utils = render(<LiveQuizCountdownTimer startsAt={startsAt} onTimeOut={onTimeOut} />);
    return { onTimeOut, ...utils };
  };

  test("does not show countdown text before first tick", () => {
    renderTimer(10);
    expect(screen.queryByText("Please wait. Quiz will be starting soon")).not.toBeInTheDocument();
  });
  ...
  test("shows countdown header after first interval tick", () => {
    ...
    expect(screen.getByText("Please wait. Quiz will be starting soon")).toBeInTheDocument();
  });
  ...
  test("handles past startsAt and times out immediately on first tick", () => {
    ...
    render(<LiveQuizCountdownTimer startsAt={startsAt} onTimeOut={onTimeOut} />);
    act(() => {
      jest.advanceTimersByTime(1000);
    });
    expect(onTimeOut).toHaveBeenCalledTimes(1);
  ...
  test("updates progress value over time", () => {
    ...
    expect(progressProps[progressProps.length - 1].value).toBeGreaterThan(progressProps[0].value);
  });
  ...
  test("restarts timer when startsAt prop changes", () => {
    ...
    const { rerender } = render(
      <LiveQuizCountdownTimer startsAt={new Date(Date.now() + 3_000)} onTimeOut={onTimeOut} />
    );
    act(() => {
      jest.advanceTimersByTime(1000);
    });
    rerender(
      <LiveQuizCountdownTimer startsAt={new Date(Date.now() + 8_000)} onTimeOut={onTimeOut} />
    );
    act(() => {
      jest.advanceTimersByTime(1000);
    });
  ...
  test("renders countdown container wrapper", () => {
    ...
    expect(container.querySelector(".live-quiz-countdown-container")).toBeInTheDocument();
  });
