...
describe("LiveQuizNavigationDots", () => {
  const renderDots = (props?: Partial<React.ComponentProps<typeof LiveQuizNavigationDots>>) =>
    render(
  ...
  ])("applies correct class for matching prior answers %#", ({ correct, selected, idx }) => {
    const { container } = renderDots({
      totalDots: 3,
      currentDot: idx,
  ...
  test.each([
    ...
    ({ correct, selected, idx }) => {
      ...
      const dots = container.querySelectorAll(".dot");
      expect(dots[0]).toHaveClass(correct[0] === selected[0] ? "correct" : "incorrect");
      expect(dots[1]).toHaveClass(correct[1] === selected[1] ? "correct" : "incorrect");
    }
  );
