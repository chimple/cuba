...
describe("HomeHeader", () => {
  ...
  test("renders home + middle icons + profile icon", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId(`header-icon-${HOMEHEADERLIST.HOME}`)).toBeInTheDocument();
      expect(
  ...
  test("clicking home icon does not trigger callback if already on home", async () => {
    ...
    const homeBtn = await screen.findByTestId(`header-icon-${HOMEHEADERLIST.HOME}`);
  ...
  test("shows profile fallback name when student has no name", async () => {
    ...
    renderHeader();
    await waitFor(() => {
      expect(screen.getByTestId(`header-icon-${HOMEHEADERLIST.PROFILE}`)).toHaveTextContent(
        "Name"
      );
    });
  });
  ...
  test("profile icon is flagged with profile marker", async () => {
    renderHeader();
    const profile = await screen.findByTestId(`header-icon-${HOMEHEADERLIST.PROFILE}`);
  ...
  test("updates stars when starsUpdated event has no studentId", async () => {
    ...
    act(() => {
      window.dispatchEvent(new CustomEvent("starsUpdated", { detail: { newStars: 31 } }));
    });
  ...
  test("profile menu closes via onClose callback", async () => {
    renderHeader();
    fireEvent.click(await screen.findByTestId(`header-icon-${HOMEHEADERLIST.PROFILE}`));
    expect(screen.getByTestId("profile-menu")).toBeInTheDocument();
  ...
  test("home and profile icons are always present regardless of link state", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId(`header-icon-${HOMEHEADERLIST.HOME}`)).toBeInTheDocument();
      expect(screen.getByTestId(`header-icon-${HOMEHEADERLIST.PROFILE}`)).toBeInTheDocument();
    });
  });
});
