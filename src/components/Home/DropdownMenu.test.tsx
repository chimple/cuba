...
describe("DropdownMenu", () => {
  ...
  const primeHomeworkCourses = () => {
    ...
    mockApi.getCourse.mockImplementation((id: string) => {
      if (id === "c1") return Promise.resolve({ id, name: "Grade Math", code: "MTH" });
      if (id === "c2") return Promise.resolve({ id, name: "Grade Science", code: "SCI" });
  ...
  test("calls onSubjectChange when selecting course in homework mode", async () => {
    ...
    const { container } = render(
      <DropdownMenu syncWithLearningPath={false} onSubjectChange={onSubjectChange} />
    );
  ...
  test("supports externally selected subject in homework mode", async () => {
    primeHomeworkCourses();
    render(
      <DropdownMenu syncWithLearningPath={false} selectedSubject="c2" />
    );
  ...
  test("filters out live quiz assignments in homework mode", async () => {
    ...
    await waitFor(() => {
      expect(container.querySelector(".dropdownmenu-dropdown-label")).toBeNull();
  ...
  test("selectedSubject fallback chooses first course if no match", async () => {
    primeHomeworkCourses();
    render(<DropdownMenu syncWithLearningPath={false} selectedSubject="unknown" />);
    await waitFor(() => {
      expect(screen.getByText("Grade Math")).toBeInTheDocument();
    });
  });
