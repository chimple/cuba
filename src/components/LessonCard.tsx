...
const LessonCard: React.FC<{
  ...
  // };
  const getDate = () => {
    const res = lesson.assignment?.updatedAt;
    if (!!res) {
      const dateObj = res?.toDate();
      setDate(dateObj);
    }
  };
  const getCurrentCourse = async () => {
    ...
    if (!currentCourse) {
      const lessonCourse = await api.getCourseFromLesson(lesson);
      if (lessonCourse) {
        setCurrentCourse(lessonCourse);
      }
    } else if (!Util.checkLessonPresentInCourse(currentCourse, lesson.docId)) {
      const lessonCourse = await api.getCourseFromLesson(lesson);
      if (lessonCourse) {
      ...
      <div>
          ...
          onClick={async () => {
            if (isUnlocked) {
              ...
              if (lesson.pluginType === COCOS) {
                ...
                const parmas = `?courseid=${subjectId}&chapterid=${lesson.cocosChapterCode}&lessonid=${lesson.id}`;
                console.log(
        ...
        >
          ...
          >
            ...
            >
              <div id="lesson-card-homework-icon" aria-hidden="true">
              ...
              {showSubjectName && currentCourse?.title ? (
                <div id="lesson-card-subject-name" aria-hidden="true">
                  <p>
                    {currentCourse?.title}
              ...
              <div id="lesson-card-image" aria-hidden="true">
                <SelectIconImage
                  localSrc={
                    "courses/" +
                ...
                />
                {!isUnlocked ? (
                  <div aria-hidden="true" id="lesson-card-status-icon">
                    <img
            ...
            <div
              aria-hidden="true"
              className="lesson-download-button-container"
            >
              <DownloadLesson
                lessonId={lesson.id}
