...
describe("SelectCourse", () => {
  ...
  it("wires slider options for subjects carousel", async () => {
    ...
    const props =
      mockSplidePropsSpy.mock.calls[mockSplidePropsSpy.mock.calls.length - 1][0];
  ...
  it("shows Add Subject button for parent mode and navigates to add-subjects", async () => {
    ...
    const addSubject = addSubjectText.closest(".subject-button") as HTMLElement | null;
  ...
  it("keeps subject slide text and data mapping consistent", async () => {
    ...
    await waitFor(() => {
      buttons = Array.from(
        document.querySelectorAll('[data-testid="splide-root"] .subject-button')
      ) as HTMLElement[];
    ...
    const mappedSlides = buttons.slice(0, 2).map((button) => ({
      ...
      grade: button
        ...
        ?.textContent?.trim(),
      curriculum: button.querySelector(".course-curriculum")?.textContent?.trim(),
    }));
  ...
  it("keeps curriculum text overflow-safe in subject cards", () => {
    const css = fs.readFileSync(
      path.join(process.cwd(), "src/components/displaySubjects/SelectCourse.css"),
      "utf8"
    );
