...
jest.mock(
  './SelectIconImage',
  () => (props: { localSrc?: string; webSrc?: string }) => (
    <div
...
jest.mock('../../services/ServiceConfig', () => ({
  ServiceConfig: {
    getI: () => ({
      apiHandler: {
        getAllCurriculums: jest
          .fn()
          .mockResolvedValue([{ id: '1', name: 'Chimple' }]),
        getAllGrades: jest
          .fn()
          .mockResolvedValue([{ id: 'g1', name: 'Grade 1' }]),
      },
    }),
  },
}));
...
describe('AddCourse Component', () => {
  ...
  test('shows checkmark icon when course is selected', async () => {
    ...
    let checkIcon = container.querySelector(
      '#add-course-subject-card-select-icon',
    );
  ...
  test('handles missing sort_index by defaulting to 0', async () => {
    ...
    render(
      <AddCourse courses={mixedSorting} onSelectedCoursesChange={jest.fn()} />,
    );
  ...
  test('renders empty grade title if grade_id is not found in map', async () => {
    ...
    render(
      <AddCourse
        courses={courseWithUnknownGrade}
        onSelectedCoursesChange={jest.fn()}
      />,
    );
    ...
    const orphanCourse = [
      {
        ...mockCourses[0],
        id: 'c2',
        curriculum_id: 'non-existent-id',
        name: 'Ghost Course',
      },
    ];

    render(
      <AddCourse courses={orphanCourse} onSelectedCoursesChange={jest.fn()} />,
    );
  ...
  test('uses default icon path when course image is missing', async () => {
    ...
    render(
      <AddCourse courses={noImageCourse} onSelectedCoursesChange={jest.fn()} />,
    );
  ...
  test('applies DEFUALT_SUBJECT_CARD_COLOUR when course color is null', async () => {
    ...
    const { container } = render(
      <AddCourse courses={noColorCourse} onSelectedCoursesChange={jest.fn()} />,
    );
    ...
    expect(iconContainer).toHaveStyle(
      `background-color: ${DEFUALT_SUBJECT_CARD_COLOUR}`,
    );
  });
  ...
  test('calls curriculum and grade APIs on mount', async () => {
    const getAllCurriculums = jest
      .fn()
      .mockResolvedValue([{ id: '1', name: 'Chimple' }]);
    const getAllGrades = jest
      .fn()
      .mockResolvedValue([{ id: 'g1', name: 'Grade 1' }]);
  ...
  test('does not render any curriculum section when API returns no curriculums', async () => {
    ...
    await waitFor(() => {
      expect(
        container.querySelector('.add-course-subject-header'),
  ...
  test('renders grade labels correctly for multiple grade ids', async () => {
    ...
    render(
      <AddCourse
        courses={coursesWithDifferentGrades}
        onSelectedCoursesChange={jest.fn()}
      />,
    );
  ...
  test('removes only the clicked course when deselecting from a multi-selection', async () => {
    ...
    render(
      <AddCourse courses={twoCourses} onSelectedCoursesChange={mockCallback} />,
    );
  ...
  test('renders one icon component per visible course', async () => {
    ...
    render(
      <AddCourse courses={threeCourses} onSelectedCoursesChange={jest.fn()} />,
    );
  ...
  test('passes per-course local icon path based on course code', async () => {
    ...
    const localSrcs = iconMocks.map((node) =>
      node.getAttribute('data-local-src'),
    );
  ...
  test('selection indicator count decreases after deselecting one course', async () => {
    ...
    fireEvent.click(scienceCard);
    expect(
      container.querySelectorAll('#add-course-subject-card-select-icon'),
    ).toHaveLength(2);
  ...
  test('does not render loading placeholder when no curriculum section is rendered', async () => {
    const unknownCourses = [
      { ...mockCourses[0], id: 'x1', curriculum_id: 'unknown' },
    ];
    render(
      <AddCourse
        courses={unknownCourses}
  ...
  test('keeps initial course list when rerendered with different courses', async () => {
    const { rerender } = render(
      <AddCourse
        courses={[{ ...mockCourses[0], id: 'c1', name: 'Maths' }]}
        onSelectedCoursesChange={jest.fn()}
      />,
    );
    expect(await screen.findByText('Maths')).toBeInTheDocument();
  ...
  test('applies mixed explicit and default icon background colors', async () => {
    ...
    const { container } = render(
      <AddCourse
        courses={mixedColorCourses}
        onSelectedCoursesChange={jest.fn()}
      />,
    );

    await screen.findByText('Science');
    const iconDivs = Array.from(
      container.querySelectorAll('.add-course-course-icon'),
    );
  ...
  test('preserves course code casing while building local icon path', async () => {
    ...
    render(
      <AddCourse courses={codedCourse} onSelectedCoursesChange={jest.fn()} />,
    );
  ...
  test('renders one clickable subject button per visible course', async () => {
    ...
    await screen.findByText('English');
    expect(
      container.querySelectorAll('.add-course-subject-button'),
    ).toHaveLength(3);
  });
  ...
  test('selecting three courses yields callback payload of length three', async () => {
    ...
    render(
      <AddCourse courses={courses} onSelectedCoursesChange={mockCallback} />,
    );
  ...
  test('selected card renders check icon class', async () => {
    ...
    fireEvent.click(await screen.findByText('Maths'));
    expect(
      container.querySelector('.add-course-gender-check-box'),
    ).toBeInTheDocument();
  });
