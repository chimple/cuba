...
jest.mock("./Home/DropdownMenu", () => () => <div data-testid="dropdown-menu" />);
...
describe("LearningPathway", () => {
  ...
  test("refreshes current student from API on mount", async () => {
    ...
    await waitFor(() => {
      ...
      expect(Util.setCurrentStudent).toHaveBeenCalledWith({ id: "stu-1", stars: 8 });
    });
  });
  ...
  test("uses resolved growthbook mode during getPath even if localStorage was pre-set", async () => {
    localStorage.setItem(CURRENT_PATHWAY_MODE, LEARNING_PATHWAY_MODE.FULL_ADAPTIVE);
    (useGrowthBook as jest.Mock).mockReturnValue({
      ready: true,
      getAttributes: () => ({}),
      setAttributes: jest.fn(),
    ...
    await waitFor(() => {
      expect(getPath).toHaveBeenCalledWith(
        expect.objectContaining({ mode: LEARNING_PATHWAY_MODE.ASSESSMENT_ONLY })
      );
    });
  });
