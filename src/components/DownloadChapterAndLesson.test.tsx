  ...
  default: ({ showDialogBox, onYesButtonClicked, onNoButtonClicked }: any) =>
    showDialogBox ? (
      <div data-testid="dialog-box-buttons">
        <button type="button" data-testid="dialog-yes" onClick={onYesButtonClicked}>
          yes
        </button>
        <button type="button" data-testid="dialog-no" onClick={onNoButtonClicked}>
          no
        </button>
      </div>
    ) : null,
...
describe("DownloadChapterAndLesson", () => {
  ...
  it("downloads all missing lesson bundles for a chapter on android", async () => {
    ...
    const view = render(
      <DownloadLesson chapter={chapter} onDownloadOrDelete={onDownloadOrDelete} />
    );
    ...
    await waitFor(() => {
      expect(mockApiHandler.getLessonsForChapter).toHaveBeenCalledWith("chapter-1");
      expect(Util.storeLessonIdToLocalStorage).toHaveBeenCalledWith(
        "chapter-1",
        DOWNLOADING_CHAPTER_ID
      );
      expect(Util.downloadZipBundle).toHaveBeenCalledWith(["cocos-2"], "chapter-1");
      expect(onDownloadOrDelete).toHaveBeenCalled();
    });
  });
  ...
  it("opens delete confirmation and deletes chapter lessons on android", async () => {
    ...
    await waitFor(() => {
      expect(mockApiHandler.getLessonsForChapter).toHaveBeenCalledWith("chapter-7");
